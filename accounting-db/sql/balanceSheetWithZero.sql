REM Select a balance sheet for a given fiscal year, displaying all accounts with a zero balance for that year.
...
HAVING NOT (SUM(CASE WHEN debit = 1 THEN i.amount*-1 ELSE i.amount END) = 0 AND MAX(fya.year) != &year) -- exclude empty accounts for earlier years
 ORDER BY 2, 1;
