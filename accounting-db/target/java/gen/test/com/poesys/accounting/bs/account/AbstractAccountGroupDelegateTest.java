...
/**
 * <p>
 * Test the AccountGroupDelegate class.
 * </p>
 * <p>
 ...
 */
public abstract class AbstractAccountGroupDelegateTest {
  /** Define a class logger. */
  protected static Logger logger = Logger.getLogger(AbstractAccountGroupDelegateTest.class);
  ...
  protected AccountGroupDelegate delegate = null;
  ...
  @Before
  public void setUp() {
    delegate = AccountDelegateFactory.getAccountGroupDelegate();
  ...
  protected void clearCaches() {
    // Primary class AccountGroup cache
    IDaoManager manager = DaoManagerFactory.getManager(getSubsystem());
    if (manager != null) {
      manager.clearCache(com.poesys.accounting.db.account.AccountGroup.class.getName());
    }
  }
   ...
   * @throws DelegateException when a problem occurs during object creation
   */
  protected List<com.poesys.accounting.bs.account.BsAccountGroup> createAccountAccountGroup(int count) 
      throws DelegateException, InvalidParametersException {
    List<com.poesys.accounting.bs.account.BsAccountGroup> objects = 
      new CopyOnWriteArrayList<com.poesys.accounting.bs.account.BsAccountGroup>();
    ...
    for (int i = 0; i < count; i++) {
      ...
      // Create the object.
      BsAccountGroup object = null;
      try {
        object = delegate.createAccountGroup(groupName);
  ...
  public void testInsert() {
    // Create a new AccountGroup object to perform the test.
    List<BsAccountGroup> objects = createAccountAccountGroup(1);
    ...
    BsAccountGroup queriedObject = queryStoredObject(key);
    ...
    for (BsAccountGroup object : objects) {
      object.delete();
    }
   ...
   */
  protected BsAccountGroup queryStoredObject(com.poesys.db.pk.NaturalPrimaryKey key)
      throws DelegateException {
    // Clear the caches, then query the saved object and test it.
    clearCaches();

    BsAccountGroup queriedObject = delegate.getDatabaseObject(key);
  ...
  protected boolean exists(com.poesys.db.pk.NaturalPrimaryKey key) throws DelegateException {
    ...
    BsAccountGroup queriedObject = delegate.getObject(key);
   ...
   *
   * @param object the BsAccountGroup object to update
   */
  protected void updateColumn(BsAccountGroup object) {
  }
   ...
   * @see #updateColumn
   */
  protected boolean isColumnUpdated(BsAccountGroup object) {
   ...
   *
   * @param original the original, updated BsAccountGroup object
   * @param queried the queried BsAccountGroup object
   * @return true if the column is updated, false if not
   */
  protected boolean isColumnUpdated(BsAccountGroup original, BsAccountGroup queried) {
    boolean retVal = true;
    return retVal;
  }
  ...
  public void testUpdateBatch() throws InvalidParametersException,
      DelegateException {
    List<BsAccountGroup> objects = createAccountAccountGroup(2);
    delegate.insert(objects);
    // Allocate a map to hold the updated objects for later comparison.
    java.util.Map<com.poesys.db.pk.IPrimaryKey, BsAccountGroup> map = 
      new java.util.TreeMap<com.poesys.db.pk.IPrimaryKey, BsAccountGroup>();
    for (BsAccountGroup object : objects) {
      updateColumn(object);
    ...
    for (BsAccountGroup object : objects) {
      BsAccountGroup queriedObject = 
        queryStoredObject((com.poesys.db.pk.NaturalPrimaryKey)object.getPrimaryKey());
      BsAccountGroup originalObject = map.get(object.getPrimaryKey());
  ...
  public void testProcess() throws InvalidParametersException,
    ...
    List<BsAccountGroup> allObjects = createAccountAccountGroup(3);
    // Insert the first two rows to later update and delete.
    List<BsAccountGroup> existingObjects = 
      new CopyOnWriteArrayList<BsAccountGroup>();
    ...
    List<BsAccountGroup> insertObject = 
      new CopyOnWriteArrayList<BsAccountGroup>();
    insertObject.add(allObjects.get(2));
    
    // Put it all together.
    List<BsAccountGroup> objects = 
      new CopyOnWriteArrayList<BsAccountGroup>(existingObjects);
    ...
    // Delete the inserted objects to clean up.
    // Mark all the objects for delete.
    for (BsAccountGroup object : allObjects) {
      object.delete();
    }
  ...
  public void testTruncateTable() {
    delegate.truncateTable("AccountGroup");
    List<BsAccountGroup> objects = delegate.getAllObjects(100);
    assertTrue("Couldn't create list", objects != null);
