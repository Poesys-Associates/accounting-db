    ...
    this might be a directory looking like this:
        ...
        (*nix)
        /home/Bob Muller/.m2/repository/accounting-db/
...
Poesys Accounting
===========================
Logging: to enable and configure log4j logging from accounting-db, you must
update the log4j.xml file under src. That will log to the file you've specified 
there and will log to the console.

The src/com/poesys/ directory contains the accounting property files and the
database property files. The AccountingBundle.properties file contains the
message strings (I18N support). The account and transaction properties files
contain the Poesys/DB subsystem specifications. The database.properties file
contains the MySQL database JDBC specifications.

The Ant build file builds the distribution, naming the output file with the
build-number value, by default 1.0.0. To build a new version, specify the
version number in a build.properties file. For example:

build-number=1.0.2

Note: the reimbursements association class, a recursive many-to-many association, 
has serious problems with code generation. As a result, the Item-related abstract
classes have been hand-coded. If you regenerate, you need to restore these from
a backup and add any additional generated code needed:

* IItem.java (interface)
* AbstractItem.java
* AbstractItemProxy.java
* AbstractBsItem.java
